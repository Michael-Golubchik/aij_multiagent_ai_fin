...
from aij_multiagent_rl_train.engine import GameEngine
...
class AijMultiagentEnv(ParallelEnv):
    ...
    def __init__(
        ...
        Args:
            stage: номер этапа тренировки
            is_one_agent: один агент или все восемь
        ...
        '''
        Доход от закидывания ресурса соседу.
        Дается двумя частями 5 взять ресурс на своей территории, и столько же при закидывании к соседу
        Если делать наоборот будет убыток. Если взять у соседа -5 и - 5 положить к себе
        '''
        # Синтетическая цена ресурса в целях обучения
        self.syn_resource_price: int = 8
        self.trash_transfer_price = 10
        self.resource_transfer_price = 2
        if self.stage == 0:
            # Учим просто собирать ресурсы и закидывать соседям
            self.ecology_penalty = 0
            # Синтетическая цена за переработку в целях обучения
            self.syn_recycle_cost = 4
        elif self.stage == 1:
            # Продолжаекм Учить просто собирать ресурсы и закидывать соседям далеко
            self.ecology_penalty = 0
            self.trash_transfer_price = 15
            self.syn_recycle_cost = 15
        elif self.stage == 2:
            # Учим собирать мусор
            self.ecology_penalty = 0
            self.trash_transfer_price = 15
        ...
        if self.is_one_agent:
            self.possible_agents = [f'agent_{i}' for i in range(1)]
            self.neighbours_mapping = {
                'agent_0': ['agent_0', 'agent_0'],
            }
        else:
    ...
    def _get_engine(
        ...
        stage=0,
        is_one_agent=False,
        trash_transfer_price = 0,
        resource_transfer_price = 0,
        syn_resource_price = 10,
        syn_recycle_cost = 4,
        ...
        engine = GameEngine(
            ...
            stage = stage,
            is_one_agent = is_one_agent,
            trash_transfer_price = trash_transfer_price,
            resource_transfer_price = resource_transfer_price,
            syn_resource_price = syn_resource_price,
            syn_recycle_cost = syn_recycle_cost,
        )            
    ...
    def reset(
        ...
        self.num_moves = 0
        self.agents = self.possible_agents.copy()
        self.engine, self.rng, self.seed = self._get_engine(
            seed=seed,
            stage=self.stage,
            is_one_agent=self.is_one_agent,
            trash_transfer_price=self.trash_transfer_price,
            resource_transfer_price=self.resource_transfer_price,
            syn_resource_price=self.resource_price,
            syn_recycle_cost=self.syn_recycle_cost,
        )
        # # Блокируем одного из агентов
        # if self.stage >= 1:
        #     bloked_agent = random.choice(self.possible_agents)
        #     self.engine.add_block(agent_id=bloked_agent)
    ...
    def state(self) -> Dict[str, np.ndarray]:
        ...
        Returns:
            Dict[str, np.ndarray]: global state with following key-value
                pairs:
                    ...
                    - 'wealth': array with agents synthetic wealth for training, shape: (8,)
        ...
        state['syn_wealth'] = np.array(
            [self.engine.agents_state[a]['syn_wealth']
             for a in self.possible_agents])
    ...
    def step(self, actions: Dict[str, int]) -> TimestampType:
        ...
        old_syn_w = {a: self.engine.agents_state[a]['syn_wealth'] for a in self.agents}
        ...
        for agent in agents:
            ...
            # if not self.engine.blocked[agent]:
            #     # Учтем в наградах число мусора у агента
            #     trash_n = self.engine.trash_by_segment(agent_id=agent)
            #     self.engine.agents_state[agent]['syn_wealth'] -= trash_n * 0.01
            #     # if trash_n >=5:
            #     #     self.engine.agents_state[agent]['syn_wealth'] -= 5
        # Get rewards from updated wealth
        rewards = {a: self.engine.agents_state[a]['wealth'] - old_w[a]
                   for a in self.agents}
        syn_rewards = {a: self.engine.agents_state[a]['syn_wealth'] - old_syn_w[a]
                   for a in self.agents}
        ...
        if truncation or any(terminations.values()):
            self.agents = []
        return observations, rewards, syn_rewards, terminations, truncations, infos
