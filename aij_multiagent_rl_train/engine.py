...
class GameEngine:
    ...
    def __init__(
        ...
        stage: int = 0,
        is_one_agent: bool = False,
        trash_transfer_price: int = 0,
        resource_transfer_price: int = 0,
        syn_resource_price: int = 10,
        syn_recycle_cost: int = 4,
    ):
        """Game Engine
        ...
        Parameters:
            ...
            stage: номер этапа
            is_one_agent: Один ли агент
            trash_transfer_price: Доход от перемещения мусора к соседу
            resource_transfer_price: Доход от взятия на борт ресурса
            syn_resource_price: Синтетический доход от переработки ресурса для обучения
            syn_recycle_cost: Синтетический убыток от утилизации мусора для обучения
        """
        ...
        self.stage = stage
        self.is_one_agent = is_one_agent
        self.trash_transfer_price = trash_transfer_price
        self.resource_transfer_price = resource_transfer_price
        self.syn_recycle_cost = syn_recycle_cost
        self.syn_resource_price = syn_resource_price
        ...
        ### -------------------------------
        # На нулевом этапе разбрасываем мусор, чтобы его было легче найти и убрать
        if self.stage <= 2:
        #if self.stage == 0:
            # На нулевом этапе разбрасываем мусор, чтобы его было легче найти и убрать
            agents_num = 1 if is_one_agent else 8
            # agents_num = 8
            for i in range(agents_num):
                num_times = random.randint(1, 10)
                # Выполнение команды self.sample_trash(agent_id=agent_id) num_times раз
                for _ in range(num_times):
                    self.sample_trash(agent_id=f'agent_{i}')
        if self.stage <= 3:
            bloked_agent = random.choice([1, 3])
            self.add_block(agent_id=f'agent_{bloked_agent}')
        # if self.stage == 1:
        #     # for i in range(2, 8):
        #     #     num_times = random.randint(1, 10)
        #     #     # Выполнение команды self.sample_trash(agent_id=agent_id) num_times раз
        ...
        #     bloked_agent = random.randint(0, 7)
        #     # bloked_agent = random.choice([0, 1])
        #     self.add_block(agent_id=f'agent_{bloked_agent}')
        
        if self.stage >= 4:
            # Блокируем одного случайного агента
            bloked_agent = random.randint(0, 7)
            self.add_block(agent_id=f'agent_{bloked_agent}')
            
            # # Блокируем двух случайных агентов
            # bloked_agent = random.randint(0, 3)
            # self.add_block(agent_id=f'agent_{bloked_agent}')
            # bloked_agent = random.randint(4, 7)
            # self.add_block(agent_id=f'agent_{bloked_agent}')
            
        if self.is_one_agent:
    ...
    def is_not_near_home_segment(self, agent_id) -> bool:
        '''
        Проверяем что сегмент не домашний и отстоит от домашнего минимум
        на один шаг
        '''
        loc = self.agents_state[agent_id]['loc']
        home_center = bool(self.segment_map[agent_id][loc[0], loc[1]].item())
        home_up = bool(self.segment_map[agent_id][max(1, loc[0]-self.move_step), loc[1]].item())
        home_down = bool(self.segment_map[agent_id][min(loc[0]+self.move_step, self.grid_size - 1), loc[1]].item())
        home_left = bool(self.segment_map[agent_id][loc[0], min(loc[1]+self.move_step, self.grid_size - 1)].item())
        home_right = bool(self.segment_map[agent_id][loc[0], max(1, loc[1]-self.move_step)].item())
        
        not_near_home = not(home_center or home_up or home_down or home_left or home_right)
        return not_near_home

    
    def is_next_neighbour_segment(self, agent_id) -> bool:
        """Check if next neighbour segment

        Check if given agent is on next neighbour segment

        Args:
            agent_id: agent ID in form `agent_{i}`

        Returns:
            bool: True if currently on next neighbour segment
        """
        next_neighbour_id = self.next_neighbours[agent_id]
        loc = self.agents_state[agent_id]['loc']
        neighbour = self.segment_map[next_neighbour_id][loc[0], loc[1]]
        return bool(neighbour.item())
    ...
    def add_block(self, agent_id: str) -> None:
        ...
        if not self.blocked[agent_id]:
            ...
            # self.agents_state[agent_id]['syn_wealth'] -= 50
            ...
            if len(valid) > 0:
                ...
                if not self.agents_state[agent_id]['inventory'][type]:
                    if type == 'resource':
                        ...
                        self.agents_state[agent_id]['syn_wealth'] += self.resource_transfer_price
                    else:
                        trash_source = self.delete_trash(center_loc=tuple(item_loc))
                        self.agents_state[agent_id]['inventory']['trash_source'] = trash_source
                        # Находится ли мусор в домашнем сегменте
                        is_trash_at_home_segment = bool(self.segment_map[agent_id][item_loc[0], item_loc[1]])
                        # Находится ли мусор на заблокированном сегменте
                        is_trash_on_blocked_segment = bool(self.blocked_map[item_loc[0], item_loc[1]])

                        # Находится ли мусор на заблокированном сегменте
                        is_trash_on_blocked_segment = bool(self.blocked_map[item_loc[0], item_loc[1]])

                        if self.blocked[agent_id]:
                            # Если агент заблокирован то забираем мусор себе
                            if self.is_home_segment(agent_id=agent_id) or is_trash_on_blocked_segment:
                                # Если берем мусор дома то убыток
                                self.agents_state[agent_id]['syn_wealth'] -= self.trash_transfer_price
                            else:
                                # Если берем мусор у соседа то доход
                                self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                                pass
                        else:
                            # Если агент не заблокирован
                            if self.is_home_segment(agent_id=agent_id):
                                # Если берем мусор дома то доход
                                self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                            else:
                                # Если берем мусор у соседа то убыток
                                self.agents_state[agent_id]['syn_wealth'] -= self.trash_transfer_price
    ...
    def throw_trash(self, agent_id: str) -> bool:
        ...
        if self.agents_state[agent_id]['inventory']['trash']:
            ...
            if valid:
                ...
                # Вычисляем куда брощен бусор
                item_loc = ag_loc * self.trash_size + self.trash_size // 2
                # Находится ли мусор на заблокированном сегменте
                is_trash_on_blocked_segment = bool(self.blocked_map[item_loc[0], item_loc[1]])
                # Находится ли мусор в домашнем сегменте
                is_trash_at_home_segment = bool(self.segment_map[agent_id][item_loc[0], item_loc[1]])
                ...
                if self.blocked[agent_id]:
                    if self.is_home_segment(agent_id=agent_id) and proprio[3] < 0.13:
                        # Если выбрасываем мусор дома и близко ц центру то доход
                        self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                        pass
                    else:
                        self.agents_state[agent_id]['syn_wealth'] -= self.trash_transfer_price
                    ...
                    if self.is_home_segment(agent_id=agent_id):
                        # Если выбрасываем мусор дома то пенальти
                        self.agents_state[agent_id]['syn_wealth'] -= self.trash_transfer_price
                    else:
                        # if (proprio[3] > 0.27
                        #     and (self.others_recycle_near(agent_id=agent_id)
                        #          or is_trash_on_blocked_segment)):
                        # if (proprio[3] > 0.27
                        #      and (self.stage <=1 or self.is_next_neighbour_segment(agent_id=agent_id))):
                        # Если выбрасываем мусор у соседа по часовой стрелке, чуть дальше от границы то доход
                        # if (proprio[3] > 0.27 and self.stage == 0
                        #     or proprio[3] > 0.37 and self.stage == 1):
                        if proprio[3] > 0.27:
                            if self.stage == 0:
                                self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                            elif self.stage >= 1:
                                ...
                                if is_trash_on_blocked_segment and self.is_not_near_home_segment(agent_id=agent_id):
                                    # Если на черное то можно кидать близко
                                    self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                                elif proprio[3] > 0.37:
                                    self.agents_state[agent_id]['syn_wealth'] += self.trash_transfer_price
                                else:
                             ...
                             # Если выбрасываем мусор у соседа, но близко к границе или не у того соседа, то убыток
                            self.agents_state[agent_id]['syn_wealth'] -= self.trash_transfer_price
        ...
        if not self.blocked[agent_id]:
            if self.agents_state[agent_id]['inventory']['resource']:
                ...
                if delta <= self.machine_reach:
                    ...
                    self.agents_state[agent_id]['syn_wealth'] += self.syn_resource_price
        ...
        if not self.blocked[agent_id]:
            ...
            if has_trash:
                ...
                if delta <= self.machine_reach:
                    ...
                    # self.agents_state[agent_id]['syn_wealth'] -= self.syn_recycle_cost
                    # done = True
                    if has_money:
                        self.agents_state[agent_id]['inventory']['trash'] = False
                        self.agents_state[agent_id]['inventory']['trash_source'] = None
                        ...
                        # Если пытаемся утилизировать мусор без денег, то штрафуем
                        pass
                        # self.agents_state[agent_id]['syn_wealth'] -= 1
        return done
    
    
    def others_recycle_near(self, agent_id: str) -> bool:
        '''
         Проверяет есть ли рядом какой-то чужой recycler
        '''
        # agent_to_exclude — это тот agent_id, которого нужно исключить из проверки
        any_within_reach = False  # Флаг для отслеживания, выполнено ли условие для хотя бы одного agent_id
        ac_loc = self.agents_state[agent_id]['loc']
        for cur_agent_id in self.recyclers.keys():
            if cur_agent_id == agent_id:
                continue  # Пропускаем нашего агента и переходим к следующему
            # print(cur_agent_id)
            
            recycler_loc = self.recyclers[cur_agent_id]['center']
            
            delta = np.abs(recycler_loc - ac_loc).max()
            
            # Проверка, если delta меньше или равна self.machine_reach
            # print("delta:", delta, 'machine_reach:', self.machine_reach*4)
            if delta <= self.machine_reach*2:
                # print("delta:", delta, 'machine_reach:', self.machine_reach*4)
                any_within_reach = True  # Устанавливаем флаг, что условие выполнено
                # print('Success') 
                break  # Можно выйти из цикла, если условие выполнено хотя бы для одного агента

        return(any_within_reach)
